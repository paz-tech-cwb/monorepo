...
export class MinistriesService {
  ...
  findAllTeams(ministryId?: number) {
    ...
    return this.teamRepo.find({
      where,
      relations: ['leader', 'coLeader', 'members', 'ministry'],
      order: { name: 'ASC' },
    });
  }
  ...
  async createTeam(dto: CreateMinistryTeamDto) {
    ...
    if (ministry.membershipMode !== 'teams') {
      throw new BadRequestException(
        'Ministry is in direct-members mode; teams are not allowed',
      );
    }
  ...
  async addMinistryMember(ministryId: number, userId: number) {
    const m = await this.ministryRepo.findOne({
      where: { id: ministryId },
      relations: ['members'],
    });
    if (!m) throw new NotFoundException();
    if (m.membershipMode !== 'direct') {
      throw new BadRequestException(
        'Ministry is in teams mode; add members to a team instead',
      );
    }
