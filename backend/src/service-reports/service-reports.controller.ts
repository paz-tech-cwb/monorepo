...
import type { RequestWithMinistryAccess } from '../ministry-access/ministry-form.guard';
...
export class ServiceReportsController {
  ...
  @Get()
  list(@Req() req: RequestWithMinistryAccess) {
  ...
  @Get(':id')
  findOne(@Param('id') id: string, @Req() req: RequestWithMinistryAccess) {
    if (!req.ministryAccess!.isLeader) {
      throw new ForbiddenException();
    }
  ...
  @Get(':id/audit')
  audit(@Param('id') id: string, @Req() req: RequestWithMinistryAccess) {
    if (!req.ministryAccess!.isLeader) {
      throw new ForbiddenException();
    }
  ...
  create(
    ...
    @Req() req: RequestWithMinistryAccess,
  ) {
  ...
  @Delete(':id')
  remove(@Param('id') id: string, @Req() req: RequestWithMinistryAccess) {
