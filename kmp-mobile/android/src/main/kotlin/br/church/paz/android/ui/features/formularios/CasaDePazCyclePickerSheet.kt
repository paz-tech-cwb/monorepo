        ...
        Column(Modifier.fillMaxWidth()) {
            ...
            when {
                ...
                state.results.isEmpty() && state.query.isNotBlank() -> Text("Nenhum resultado")
                    ...
                    val cycles = state.results as List<CasaDePazCycle>
                    LazyColumn(Modifier.heightIn(max = 300.dp)) {
                        items(cycles) { cycle ->
                            ListItem(
                                headlineContent = { Text(cycle.name) },
                                trailingContent = if (cycle.id == selectedId) {
                                ...
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .clickable { onSelect(cycle.id, cycle.name) },
                            )
                            HorizontalDivider()
                        }
                    }
