...
export class AreaSupervisorReportsService {
  constructor(
    @InjectRepository(AreaSupervisorReport) private readonly repo: Repository<AreaSupervisorReport>,
    private readonly policy: FormSubmissionPolicyService,
    private readonly audit: FormSubmissionAuditService,
  ) {}

  async create(dto: CreateAreaSupervisorReportDto, actorId: number): Promise<AreaSupervisorReport> {
  ...
  async update(id: string, dto: UpdateAreaSupervisorReportDto, actor: { id: number; roleSlug: string }): Promise<AreaSupervisorReport> {
