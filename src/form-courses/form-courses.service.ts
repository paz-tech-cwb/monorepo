...
export class FormCoursesService {
  constructor(
    @InjectRepository(FormCourse) private readonly courses: Repository<FormCourse>,
    @InjectRepository(FormCourseLink) private readonly links: Repository<FormCourseLink>,
  ) {}

  async listForForm(formSlug: string): Promise<Array<FormCourse & { display_order: number }>> {
    const linked = await this.links.find({ where: { formSlug }, order: { displayOrder: 'ASC' } });
    if (!linked.length) return [];
    const courseIds = linked.map(l => l.courseId);
    const courses = await this.courses.findBy(courseIds.map(id => ({ id, isActive: true })));
    return linked
  ...
  async createAndLink(formSlug: string, dto: CreateFormCourseDto): Promise<FormCourse> {
    const course = await this.courses.save(
      this.courses.create({ name: dto.name, description: dto.description ?? null, isActive: dto.isActive ?? true }),
    );
  ...
  async update(id: string, dto: UpdateFormCourseDto): Promise<FormCourse> {
    ...
    Object.assign(c, { name: dto.name ?? c.name, description: dto.description ?? c.description, isActive: dto.isActive ?? c.isActive });
