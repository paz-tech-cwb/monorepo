...
export class ServiceReportsService {
  constructor(
    @InjectRepository(ServiceReport) private readonly repo: Repository<ServiceReport>,
    private readonly policy: FormSubmissionPolicyService,
    private readonly audit: FormSubmissionAuditService,
  ) {}

  async create(dto: CreateServiceReportDto, actorId: number): Promise<ServiceReport> {
  ...
  async update(id: string, dto: UpdateServiceReportDto, actor: { id: number; roleSlug: string }): Promise<ServiceReport> {
